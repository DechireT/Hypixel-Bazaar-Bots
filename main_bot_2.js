...
client.ws.on('INTERACTION_CREATE', async interaction => {
    ...
    if(interaction.data.custom_id == null) {
        ...
        if(command.startsWith('help')) {
            ...
            for (let i = 75; i < CommandSize; i++) {
                emblem4.addField('**/' + commands[i].name + '**', commands[i].description + '\n')
            }
            
            emblem2.setColor('#ffff00');
            emblem3.setColor('#ffff00');
            emblem4.setColor('#ffff00');       
            emblem4.setTimestamp();
            emblem4.setFooter("By DechireT#0674");
            sendApi(interaction, emblem);
            const channel = client.channels.cache.get(interaction.channel_id);
            channel.send(emblem2);
            channel.send(emblem3);
            channel.send(emblem4);
            update_guild_user();
